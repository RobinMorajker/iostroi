...
const refRate = 0.05;
const teamRate = 0.05;       // for developer
const promoteRate = 0.05;

const minDeposit = 10;

const contractTeam = "roiteam"; // 开发者账户  
const contractPromote = "roiteam"; // 推广账户  
...
class LuckyBet {
    ...
    _getRealReffer(account, reffer) {
        ...
        if (reffer == "" || reffer == account) {
            reffer = contractPromote; //blockchain.contractOwner();
        }
    ...
    deposit(account, amount, reffer) {
        ...
        blockchain.callWithAuth("token.iost", "transfer", ["iost", blockchain.contractName(), contractTeam, nRewardBalance, "LuckyDeposit|reward for developer"]);
